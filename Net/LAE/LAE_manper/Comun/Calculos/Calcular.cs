...
namespace LAE.Comun.Calculos
{
    public class Calcular
    {
        ...
        // TODO Usar nombres con sentido como aquí!
        ...
        public static Valor CHN_CCI_8_5(Valor valorCCI, params Valor[] valores)
        {
            if (valores != null && valores.Count() > 0)
            {
                Valor promedio = Promedio(valores);
                return Valor.Of(Math.Abs((promedio.Value - valorCCI.Value) * 100 / valorCCI.Value), "%");
            }
            return Valor.Of(0, "");
        }

        public static Valor PCS_8_6_vbhCorreccion(Valor PCSbvh, Valor masa, Valor Qmecha, Valor NO3, Valor SO4)
        {
            //ACABAR METODO
            Valor.Conversor.BatchConvert("MJ/Kg", PCSbvh);
            Valor.Conversor.BatchConvert("Gramos", masa);
            Valor.Conversor.BatchConvert("Julio", Qmecha);
            Valor.Conversor.BatchConvert("mg/l", NO3);
            Valor.Conversor.BatchConvert("mg/l", SO4);
            //double correccionNO3=NO3.Value*

            return Valor.Of(((PCSbvh.Value * masa.Value * 1000) - Qmecha.Value - NO3.Value - SO4.Value) / (masa.Value * 1000), 0);

        }

        public static Valor Fusibilidad_CCI_8_7(Valor valorCCI, Valor valorTemperatura)
        {
            Valor.Conversor.BatchConvert("ºC", valorCCI, valorTemperatura);
            return DiferenciaAbsoluta(valorCCI, valorTemperatura);
        ...
        public static Valor PromedioCorreccionHumedad(ValorHumedad[] valoresPorcentaje, Func<double?, double?, double?> funcPorcentajeSeca)
        {
            if (valoresPorcentaje != null && valoresPorcentaje.Count() > 0 && !valoresPorcentaje.Any(v => v.Humedad3 == null))
            {
                double? valor = valoresPorcentaje.Average(v => funcPorcentajeSeca(v.Valor.Value, v.Humedad3));
                return Valor.Of(valor, valoresPorcentaje.FirstOrDefault().Valor.Unidad);
            }
            return null;
        }

        public static Valor PromedioCorreccionHumedad(ValorHumedad[] valoresPorcentaje, Func<double?, double?, double?> funcPorcentajeSeca, Func<double?, double?, double?> funcPorcentajeHumeda)
        {
            if (valoresPorcentaje != null && valoresPorcentaje.Count() > 0 && !valoresPorcentaje.Any(v => v.Humedad == null || v.Humedad3 == null))
            {
                /*seco -> hum3*/
                valoresPorcentaje.ForEach(v => v.Valor = Valor.Of(funcPorcentajeSeca(v.Valor.Value, v.Humedad3), v.Valor.Unidad));
                /*humedezco -> hum*/
                double? valor = valoresPorcentaje.Average(v => funcPorcentajeHumeda(v.Valor.Value, v.Humedad));
                return Valor.Of(valor, valoresPorcentaje.FirstOrDefault().Valor.Unidad);
            }
            return null;
        }
        ...
        public static ResultadoInforme Resultado(double valueMayor, int? numDecimal = null)
        {
            ResultadoInforme resultado = new ResultadoInforme();
            resultado.Incertidumbre = null;
            resultado.Valor = ">" + VisualizeDecimals(valueMayor, numDecimal ?? 0);
