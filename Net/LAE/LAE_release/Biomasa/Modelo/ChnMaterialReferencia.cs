...
namespace LAE.Biomasa.Modelo
{
    public class FactoriaChnMaterialesReferencia
    {
        ...
        public static ChnMaterialReferencia[] GetMaterial(int idControl, bool? certificado = null)
        {
            String consulta = @"SELECT id_chnmaterialreferencia Id
                                    FROM biomasa.chn_materialesreferencia
                                    INNER JOIN materiales_referencia ON idmaterial_chnmaterialreferencia = id_materialreferencia
                                    WHERE (certificado_materialreferencia=@Certificado OR @Certificado IS NULL)
                                    AND usable_materialreferencia = TRUE
                                UNION
                                SELECT id_chnmaterialreferencia Id
                                    FROM biomasa.chn_materialesreferencia
                                    INNER JOIN biomasa.chn_control ON id_chnmaterialreferencia = idmaterialreferencia_chncontrol
                                    WHERE id_chncontrol = :IdControl";
            try
            {
                using (NpgsqlConnection conn = PersistenceDataBase.GetConnection())
                {
                    return conn.Query<ChnMaterialReferencia>(consulta, new { IdControl = idControl, Certificado = certificado })
                        .Map(m =>
                        {
                            m.Load(conn);
                            return m;
        ...
        public static ChnMaterialReferencia[] GetMaterialDeriva(int idChnDeriva, bool? certificado = null)
        {
            String consulta = @"SELECT id_chnmaterialreferencia Id
                                    FROM biomasa.chn_materialesreferencia
                                    INNER JOIN materiales_referencia ON idmaterial_chnmaterialreferencia = id_materialreferencia
                                ...
                                SELECT id_chnmaterialreferencia Id
                                    FROM biomasa.chn_materialesreferencia
                                    INNER JOIN biomasa.chn_deriva ON id_chnmaterialreferencia = idmaterialreferencia_chnderiva
                                    WHERE id_chnderiva = :IdChnDeriva";
            try
            {
                using (NpgsqlConnection conn = PersistenceDataBase.GetConnection())
                {
                    return conn.Query<ChnMaterialReferencia>(consulta, new { IdChnDeriva = idChnDeriva, Certificado = certificado })
                        .Map(m =>
                        {
                            m.Load(conn);
                            return m;
