...
namespace Cartif.Extensions
{
    ...
    public static class StringExtension
    {
        ...
        {
            StringBuilder pascalCaseVersion = new StringBuilder();

            foreach (string segment in theString.SplitWords())
            {
                pascalCaseVersion.Append(segment.Capitalize());
            }

            return pascalCaseVersion.ToString();
        }
        ...
        {
            theString.ThrowIfArgumentIsNull("Can't capitalize an empty string.");
            
            return char.ToUpperInvariant(theString[0]) + (theString.Length > 1 ? theString.Substring(1) : "");
        }
