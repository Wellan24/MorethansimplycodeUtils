...
namespace Cartif.Logs
{
    ...
    public class Log
    {
        ...
        private const String defaultFormat = "[ {1:dd/MM/yyyy HH:mm:ss} || {2} => {3} ]  Mensaje: {4}\n{5}";    /* The mensaje */
        ...
        ///--------------------------------------------------------------------------------------------------
        public Log(TipoLog tipo, MethodBase method, String log)
        {
            Method = method;
            MensajeLog = log;
            Fecha = DateTime.Now;
        ...
        ///--------------------------------------------------------------------------------------------------
        public Log(TipoLog tipo, MethodBase method, String log, Exception ex)
            : this(tipo, method, log)
        {
            Excepcion = ex;
        }
        ...
        public string ToString(String format)
        {
            try
            {
                ...
                return String.Format(format, TipoLog, Fecha, Method.DeclaringType.FullName, Method.Name, MensajeLog, Excepcion != null ? "\n" + Excepcion : "");
            }
            catch (Exception)
            {
                throw;
