...
namespace LAE.Modelo
{
    public class FactoriaMedicionPNT
    {
        ...
        {
            if (addFechaFin)
                return new MedicionPNT() { FechaInicio = DateTime.Now, FechaFin = DateTime.Now.AddDays(1), AddFechaFin = true, IdTecnico = idTecnico, IdMuestra = idMuestra };
        ...
        {
            if (addFechaFin)
                return new MedicionPNT[] { new MedicionPNT { FechaInicio = DateTime.Now, FechaFin = DateTime.Now.AddDays(1), AddFechaFin = true, IdTecnico = idTecnico, IdMuestra = idMuestra } };
            return new MedicionPNT[] { new MedicionPNT { FechaInicio = DateTime.Now, IdTecnico = idTecnico, IdMuestra = idMuestra } };
        }

        public static MedicionPNT GetMedicion(int idMuestra, String tabla, String id, bool addFechaFin = false)
        {
            StringBuilder consulta = new StringBuilder(@"SELECT id_medicionpnt Id, fechainicio_medicionpnt FechaInicio, fechafin_medicionpnt FechaFin, idtecnico_medicionpnt IdTecnico, observaciones_medicionpnt Observaciones, idmuestra_medicionpnt IdMuestra
                                    FROM medicion_pnt
            ...
            try
            {
                using (NpgsqlConnection conn = PersistenceDataBase.GetConnection())
                {
                    ...
                    if (medicion != null)
                        medicion.AddFechaFin = addFechaFin;
                    return medicion;
                }
            }
            catch (Exception ex)
        ...
        public static MedicionPNT[] GetMediciones(int idMuestra, String tabla, String id, bool addFechaFin = false)
        {
            StringBuilder consulta = new StringBuilder(@"SELECT id_medicionpnt Id, fechainicio_medicionpnt FechaInicio, fechafin_medicionpnt FechaFin, idtecnico_medicionpnt IdTecnico, observaciones_medicionpnt Observaciones, idmuestra_medicionpnt IdMuestra
                                    FROM medicion_pnt
                                    LEFT JOIN medicion_pntcci ON id_medicionpnt=idcci_medicionpntcci
            ...
            try
            {
                using (NpgsqlConnection conn = PersistenceDataBase.GetConnection())
                {
                    ...
                    Array.ForEach(mediciones, m => m.AddFechaFin = addFechaFin);
    ...
    public class MedicionPNT : PersistenceData, IModelo
    {
        ...
        [ColumnProperties("fechafin_medicionpnt")]
        ...
        public bool AddFechaFin { get; set; }
    }
}