...
namespace Cartif.Util
{
    public static class ReflectionUtils
    {
                                                                                         ...
                                                                                         where TReturn : class
        {
            ...
            return typeof(TReceive)?.GetProperty(name, BindingFlags.Public | BindingFlags.Static)?.GetValue(null, null) as TReturn;
        }
        ...
        {
            // Use the PropertyInfo to retrieve the value from the type by not passing in an instance
            return typeof(TReturn)?.GetProperty(name, BindingFlags.Public | BindingFlags.Instance)?.GetValue(obj, null) as TReturn;
        }

        public static Object GetValueOfPropertyOf(this Type type, String name, Object obj)
        {
            // Use the PropertyInfo to retrieve the value from the type by not passing in an instance
            return type?.GetProperty(name, BindingFlags.Public | BindingFlags.Instance)?.GetValue(obj, null);
        }
    }
}
