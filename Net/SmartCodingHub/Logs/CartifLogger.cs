        ...
        public void GenerarLog(TipoLog tipoLog, MethodBase method, String log, Exception exception, bool esInmediato)
        {
            try
            {
                if (tipoLog != null)
                {
                    ...
                    ColaLogs[tipoLog].Enqueue(new Log(method, log, exception));
                    NumeroLogs++;
            ...
            {
                CartifLogs.GenerarLog(TipoLog.From("Logs"), "Problema generando logs. TipoLog=" + tipoLog.Nombre + " log=" + log, true);
            }
        }
